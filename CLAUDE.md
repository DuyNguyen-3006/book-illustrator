...
| Frontend | React + Vite (TypeScript) | Matches `app-demo.html` reference scope. Server state: **TanStack Query** (polling rules in `frontend-rules` §2 map onto it directly). Routing: **React Router**. Styling: **Tailwind CSS v3** + **Lightswind** components copied into `src/components/ui` — v3 not v4 because Lightswind ships a v3 plugin; its peers (`clsx`, `tailwind-merge`, `class-variance-authority`, `lucide-react`) are part of that choice. Colours/radii/type come from the token layer in `src/index.css`, not from Lightswind's defaults. Tests: **Vitest** + Testing Library. Layout is feature-sliced (`features/<domain>/{components,pages,hooks,services,types}`, `shared/`, `router/`, `config/`) — see `DECISIONS.md` §13 |
...
- **`design-taste-frontend`** — the design language for this app: token layer, type scale,
  colour calibration, interactive states, dark mode, and its list of AI tells. Its own §13
  says it is **not** for multi-step product UI, which is exactly what the pipeline screens
  are — so apply its language and pre-flight checks, not its landing-page section rules
  (no hero/eyebrow/marquee patterns on pipeline screens). Hard limits: **no new
  dependencies** (§2.1) — no GSAP, no icon library swap, `lucide-react` stays because it
  already ships with Lightswind; and `frontend-rules` wins on behaviour (layout-stable
  skeletons, distinct loading/error/empty states, no invented progress bars).
